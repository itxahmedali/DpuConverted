...
function resize() {
	canvasW = document.getElementById('containerCST').offsetWidth + 40; initCanvas(canvasW, window.innerHeight);
	var cW = canvas.width;
...
bodyScrollBar.addListener(() => {
	...
	var con = Utils.isElementInView($('.sendBtn'), false);
	if (con) {
		$(".cito").css('display','none');
		autoDiff = 500;
		xx = 1 + Math.floor((verNum - 2) * (window.innerWidth-100) / canvas.width);
